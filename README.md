...
2. 点击导航栏"用户管理"
3. 可执行的操作：
   - **新建用户**：创建新的作者/编辑/管理员账号
   - **编辑用户**：修改用户信息、角色、状态
   - **删除用户**：删除用户（不能删除当前登录用户）

#### 作者页面

点击文章作者名称或头像，可以查看该作者的所有文章。
...
### 11. 如何复制代码块？
在文章页面，代码块右上角有复制按钮（📋）。点击后会自动复制代码到剪贴板，并显示"代码已复制"提示。

### 12. 如何放大查看图片？
点击文章中的任意图片，会弹出灯箱效果，显示放大的图片。支持：
- 左右箭头键切换图片
- 点击左右按钮切换
- ESC 键关闭
- 移动端左右滑动切换
...
### 14. 文章标题固定在顶部如何解决？
文章页面已优化，标题和作者信息不再固定在顶部，随页面自然滚动，提供更好的阅读体验。

### 15. 图片上传失败提示 413 错误？
图片大小可能超过限制。当前限制为 16MB。可以在 `backend/config.py` 中修改 `MAX_CONTENT_LENGTH` 调整限制。
...
#### 用户体验提升
- ✨ 代码一键复制功能（支持 Clipboard API）
- ✨ 自定义图片灯箱效果（点击放大、键盘导航、触摸滑动）
- ✨ 文章页面优化（标题不固定，沉浸式阅读）
- ✨ 作者/时间信息左右布局优化
