# Simple Blog 文档中心

完整的文档索引和导航指南。

## 🧭 功能总览

- [功能总览（Feature Catalog）](features.md) - 全系统功能清单、访问入口、快捷键与配置前置

## 🚀 快速开始

- [快速启动](startup.md) - 详细的环境配置和启动步骤
- [快速升级参考](../QUICKSTART.md) - 一行命令快速升级与启动
- [前端构建指南](frontend-build.md) - React/Vite 知识库编辑器构建说明
- [知识库重构说明](knowledge-base-refactor.md) - 新版独立知识空间介绍

## 📦 部署运维

- [部署指南](../DEPLOYMENT.md) - 完整的部署和升级指南
- [启动指南](startup.md) - 系统启动和配置说明

## ✨ 个人效率功能

- [个人效率功能说明](personal-features.md) - 命令面板/语义搜索/回顾页/快捷捕捉等 9 项功能的使用与配置

## 🔄 升级迁移

- [升级指南](upgrade.md) - 功能升级步骤
- [个人效率功能版升级部署指南](upgrade-2026-09-personal-features.md) - 2026-09 版本（命令面板/语义搜索/回顾页/快捷捕捉）升级步骤、验证与回滚
- [Passkey 升级手册](passkey-upgrade.md) - 快捷登录功能升级说明

## 🧪 测试文档

- [测试说明](testing.md) - 测试运行方式与测试结构

## 🔒 安全

- [Passkey 操作手册](passkey-operations.md) - Face ID / Touch ID 快捷登录使用说明

## 🏗️ 架构说明

- [架构说明](ARCHITECTURE.md) - 模块划分与关键流程

## 📚 API 文档

- [API 文档](api-documentation.md) - 完整的 REST API 参考

## 🔧 工具文档

- [Backend 工具](../backend/README.md) - 图片清理等工具使用说明
- [前端构建指南](frontend-build.md) - 新版知识库编辑器构建与排错

## 🗃️ 历史归档

- [归档报告](archive/reports/) - 优化、验收与阶段性总结文档

---

**提示**: 如果您是第一次使用，建议从 [快速启动](startup.md) 开始；如果需要使用新版知识库编辑器，请先阅读 [前端构建指南](frontend-build.md)。
